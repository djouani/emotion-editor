...
function print_event () {
  return "Tapahtuma " + this.time + ": Toimija " + this.causing_object.oid + " antaa toimijalle " + 
         this.target_object.oid + " hyötymäärän " + this.utility
}
...
function start_life () {
  document.getElementById("results").innerHTML = "Elämä alkaa: toimijoiden lukumäärä on "+this.objects.length+"."

  for (var i = 0; i < this.objects.length; i++) {
    document.getElementById("object" + (i+1) + "results").innerHTML = "Toimija "+(i+1)+":<br /><br />" + this.objects[i].controller.toString() 
    for (var j = 0; j < this.objects.length; j++) {
      if (i != j) { 
        document.getElementById("result"+(i+1)+(j+1)).innerHTML = "tuntematon"
      }
    }
  }
  return 0;
...
function process_user_event (causing_object, target_object, utility) {
  ...
  for (var i = 0; i < this.objects.length; i++) {
    document.getElementById("object" + (i+1) + "results").innerHTML = "Toimija " + (i+1) + ":<br />" + this.objects[i].process_event(ev) + "<br />" + this.objects[i].controller.toString() 
